...
class Setup():
    ...
    def DAS(self, mic_data, xmax=5,ymax=5,resolution=200, display=True):
        ...
        mx = -10000
        zmx = np.full((self.xRES,self.yRES),-1)
        # print(self.mics)
...
# setup.plot_mic_data()
a = 0.5
setup.DAS(rx2,ymax=5,xmax=20,resolution=100,display=True)
setup.plot_DAS()
setup.plot_3D()
...
setup.find_mic_data()
setup.display_wave_output()
setup.DAS(setup.mic_data,ymax=5,xmax=10,resolution=100,display=True)
setup.plot_DAS()
setup.plot_3D()
# answer : 3.075, -1.0
'''