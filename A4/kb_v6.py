...
class Key:
    def __init__(self, ind, char,row,col,x,y,width, shift_char=None,freq=0):
        self.ind = ind
        self.char = char
...
class Keyboard():
    def __init__(self, listed_keyboard_layout, key_size):
        ...
        for i,row in enumerate(self.listed_keyboard_layout):
            ...
            for j,key_chars in enumerate(row):
                ...
                width = self.key_size.get(key_chars[0], 1)
                
                key = Key(index, normal_char, shift_char=shift_char,row=i,col=j,x=x_pos,y=y_pos,width=width)
        ...
        self.x_end = x_pos-0.1
        self.y_end = y_pos+1.1
    ...
    def create_radial_gradient(self,grayscale,xcentre,ycentre,xradius,yradius):
        ...
        rx = int(self.xRES*xradius/(self.x_end-self.x_start))
        ry = int(self.yRES*yradius/(self.y_end-self.y_start))

        for x in range(x0-2*rx,x0+2*rx):
            if x<0 or x>=self.xRES:
                continue
            for y in range(y0-2*rx,y0+2*rx):
                if y<0 or y>=self.yRES:
                    continue
                r = np.sqrt((x-x0)**2+(y-y0)**2)
                
                val = grayscale*expo(r)
                
                alpha = grayscale*expo(r*2)
                self.grey_mask[-y,x] = min(1,val+self.grey_mask[-y,x])
                self.alpha_mask[-y,x] = min(1,alpha+self.alpha_mask[-y,x])
        
    def colorize(self):
        n,m = self.grey_mask.shape
        # print(self.grey_mask[200,200])
        for i in range(n):
            for j in range(m):
                val = self.grey_mask[i][j]
                ...
                alpha = self.grey_mask[i][j]
                self.color_mask[i][j] = (R,G,B,alpha)
        ...
        for rowi in range(len(self.listed_keyboard_layout)):
            for j in range(len(self.listed_keyboard_layout[rowi])):
                ...
                if freq:
                    ...
                    offset = 0.6
                    self.create_radial_gradient(grayscale=(freq/self.THxRESH),xcentre=x_pos+width/2,ycentre=y_pos+1/2,xradius=width/2+offset,yradius=10+offset)
                ...
                if shift_key != None and not shift_key.isalpha():
                    self.ax.text(x_pos + width / 5, y_pos + 0.75, shift_key.upper(), ha='center', va='center', fontsize=10,font=KEY_FONT,zorder=Zor)
                    Zor+=1
        ...
        while True:
            while i<len(s):
                ...
                if key.shift_char==s[i]:
                    ...
                    self.THxRESH = max(self.THxRESH,self.keyboard_layout[self.key_index['SHIFT']].freq)
                i+=1
...
keyboard = Keyboard(qwerty_keyboard_layout,qwerty_key_size)
