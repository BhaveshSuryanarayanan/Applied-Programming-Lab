...
class Keyboard():
    ...
    def draw_keyboard_layout(self):
        ...
        self.clr = self.ax.imshow(self.color_mask, extent=(self.x_start,self.x_end,self.y_end, self.y_start), origin='lower',zorder=5)
        self.ax.set_xlim(self.x_start, self.x_end )
        self.ax.set_ylim(self.y_end, self.y_start)
        self.ax.set_aspect('equal')
        self.ax.axis('off')
    ...
    # def press_key(frame,c):
    #     for key in keys:
    #         key.set_facecolor('white')
    def run(self,s = ""):
        ...
        while True:
            while i<len(s):
                ...
                scale = 1.5
                rect.set_zorder(4)
                rect.set_bounds(xpos+w/2-w*scale/2, ypos+h/2-h*scale/2, w*scale, h*scale)
                ...
                if key.shift_char==s[i]:
                    ...
                    shift_rect.set_edgecolor('white')
