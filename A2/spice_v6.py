...
def evalSpice(filename):
    ...
    # List of the nodes in the circuit
    nodes = ['GND']
    ...
    # Save the index of the nodes in the list in dict for easy accessing
    label = {}
    ...
    # Initialise Adjacency list to store the connections to each node 
    circuit = [[] for _ in range(len(nodes))]

    # Iterate through the file again to intake components
    for line in lines:
        if '.circuit' in line:
            start = 1
            continue
    ...
    # INF = float('inf')
    # def Isource_check(node):
    #     if visited_nodes[node]:
    #         return -INF
    #     visited_nodes[node]+=1
    #     r = -INF
    #     for el in circuit[node]:
    #         r = max(r,Isource_check(el[0]))
...
filename = r"D:\sem3\APL\A2\testcases\tc2.txt"
evalSpice(filename)
