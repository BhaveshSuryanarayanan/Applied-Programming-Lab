...
def evalSpice(filename):
    ...
    for line in lines:
        ...
        if words[0][0].upper() == 'V':
            vsource_dict[words[0]] = Vsource(words[0][0],float(words[-1]),label[n1],label[n2])
    ...
    for i in range(len(keys)):
        for j in range(i+1,len(keys)):
            ...
            if v1.n2==v2.n1 and v1.n2==v2.n1 and v1.val!=-v2.val:
                raise ValueError('Circuit error: no solution')
    ...
    def find_current(Vname):
        vsource = vsource_dict[Vname]
        val = vsource.val
        n1 = vsource.n1
        n2 = vsource.n2
        
        visited_sources[Vname]+=1
        
        return find_node_current(n1)
        

    visited_sources = defaultdict(int)

    def find_node_current(n):
        cur = 0
        for el in circuit[n]:
            n1 = n
            n2 = el[0]
            ...
            val = comp.value
            if comp.type == 'R':
                cur+=(node_voltage[n1]-node_voltage[n2])/val
            elif comp.type == 'I':
                cur+=val
            else:  
                if not visited_sources[comp.name]:
                    cur+=find_current[comp.name]
        return cur
    
    print('yes')
    vsource_current = {}
    for v in vsource_dict:
        vsource_current[v] = -find_current(v)
...
# filename = r"D:\sem3\APL\A2\testdata\test_invalid_element.ckt"
# evalSpice(filename)